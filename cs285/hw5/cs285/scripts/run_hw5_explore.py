...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if step % args.log_interval == 0:
            for k, v in update_info.items():
                if isinstance(v, (int, float)) or (hasattr(v, "item")):
                    try:
                        logger.log_scalar(float(v), k, step)
                    except Exception:
                        pass
            logger.flush()

        # if step % args.eval_interval == 0:
        #     trajectories = utils.sample_n_trajectories(env, agent, args.num_eval_trajectories, ep_len)
        ...
        #     observations_np = np.asarray(recent_observations, dtype=np.float32)
        #     recent_observations = []
        #     if observations_np.ndim == 1:
        #         observations_np = observations_np[None, :]
        #     observations_np = np.ascontiguousarray(observations_np, dtype=np.float32)
        #     fig = visualize(env_pointmass, agent, observations_np)
        #     # logger.log_figure(fig, "exploration_trajectories", step, "eval")
...
def main():
    ...
    parser.add_argument("--visualize_interval", "-vi", type=int, default=100000000)
