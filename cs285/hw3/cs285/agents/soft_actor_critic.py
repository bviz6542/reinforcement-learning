...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            next_qs = torch.flip(next_qs, dims=[0])
    ...
    def update_critic(
        ...
        """
        (batch_size,) = reward.shape if reward.ndim == 1 else (reward.shape[0],)
        reward = reward.view(batch_size)
        done = done.view(batch_size).to(dtype=reward.dtype)
        not_done = 1.0 - done
        ...
        with torch.no_grad():
            ...
            next_action_dist: torch.distributions.Distribution = self.actor(next_obs)
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                try:
                    next_action_entropy = self.entropy(next_action_dist)
                except Exception:
                    a = next_action_dist.sample()
                    next_action_entropy = -next_action_dist.log_prob(a)
                    if next_action_entropy.ndim > 1:
                        next_action_entropy = next_action_entropy.sum(dim=-1)
                next_qs = next_qs + self.temperature * next_action_entropy[None, :]

            # Compute the target Q-value
            target_values: torch.Tensor = reward[None, :] + not_done[None, :] * self.discount * next_qs
        ...
        # Compute loss
        loss: torch.Tensor = ((q_values - target_values) ** 2).mean()
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            qs_flat = self.critic(obs_flat, act_flat)
            q_values = qs_flat.view(self.num_critic_networks, self.num_actor_samples, batch_size)
        ...
        loss = (self.temperature * log_probs - advantage).mean()
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        q_values_all = self.critic(obs, action)
        if self.num_critic_networks >= 2:
            q_values = torch.min(q_values_all, dim=0).values
        else:
            q_values = q_values_all.mean(dim=0)

        # TODO(student): Compute the actor loss
        logp = action_distribution.log_prob(action)
        if logp.ndim > 1:
            logp = logp.sum(dim=-1)
        loss = (self.temperature * logp - q_values).mean()
    ...
    def update(
        ...
        elif self.target_update_period is not None and self.target_update_period > 0:
