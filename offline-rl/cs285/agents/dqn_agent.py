...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        if np.random.rand() < epsilon:
            action = np.array([[np.random.randint(self.num_actions)]], dtype=np.int64)
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                next_actions_online = torch.argmax(self.critic(next_obs), dim=-1)
                next_q_values = next_qa_values.gather(1, next_actions_online.unsqueeze(1)).squeeze(1)
            else:
                next_q_values, _ = torch.max(next_qa_values, dim=-1)

            not_done = 1.0 - done.to(reward.dtype).squeeze(-1)
            target_values = reward.squeeze(-1) + not_done * self.discount * next_q_values
